...
// export default Customize_trip_pdf
...
const FullyCustomizePdf = ({ data }) => {
    ...
    data.forEach((entry) => {
        content.push(
            <View key={entry.day} style={styles.section}>
                <Text style={styles.header}>Day {entry.day}</Text>
                <Text style={styles.text}>Location: {entry.selectedLocation}</Text>
                <Text style={styles.header}>Selected Activities:</Text>
                {entry.selectedActivities.map((activity, activityIndex) => (
                    <Text key={activityIndex} style={styles.activityList}>
                        - {activity.label}
    ...
    return (
        <div>
            <button>
                ...
                >
                    {({ loading }) => (loading ? 'Preparing document...' : 'Download PDF of Trip')}
                </PDFDownloadLink>
            </button>
        </div>
    );
